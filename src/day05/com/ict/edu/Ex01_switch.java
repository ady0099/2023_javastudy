...
public class Ex01_switch {
	...
	public static void main(String[] args) {
		/* switch ~ case : 다중 if~else 문과 같은 조건문
		  ...
		  switch문 int, char, Stgring 일 때 사용 (long, float, double문 사용 불가)
		...
		System.out.println("break문이 없는 경우");
		int k2 = 2;
		switch (k2) {
		...
		System.out.println("================");
		System.out.println("case char & 중복코드 지우기!!");
		// char k3이 1, A, a이면 아프리카, 2, B, b이면 브라질, 3, C, c이면 캐나다 나머지 한글
		char k3 = 'x';
		int ran2 = (int)(Math.random()*3)+1;
		if(k3=='x') {
			switch(ran2) {
				case 1 : ran2=(char)((Math.random()*3)+97); k3=(char)(ran2); break;
				case 2 : ran2=(char)((Math.random()*3)+49); k3=(char)(ran2); break;
				case 3 : ran2=(char)((Math.random()*3)+65); k3=(char)(ran2); break;
			}
		}
		switch(k3) {
			case '1' :	
			case 'A' : // res = "아프리카"; break; 
			case 'a' : res = "아프리카"; break;  	//중복코드 지우기!! 같은 것끼리 묶어주면 ||(or) 처럼 사용 가능
			case '2' :
		...
		// 만약에 가독성을 생각한다면
		// case '1' :	case 'A' :  case 'a' : res = "아프리카"; break;  	
		// 이런식으로 1~2줄로 처리하면 될 것 같다
		...
		switch (k6) {
			...
			// 범위형일 경우 switch문은 번거로움
			// 하지만 (int) /10과 같은 방식으로 작업량을 줄일 수는 있음 
		}
		
		switch ((int)(k6/10)){
			case 10: case 9: res = "A학점"; break;
