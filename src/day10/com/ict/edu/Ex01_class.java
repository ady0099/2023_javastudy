...
public class Ex01_class {
	// 클래스
	// 자바에서는 클래스는 객체(Object)를 정의하는 틀 또는 설계도면과 같은 의미로 사용됨
	// 객체란 클래스를 바탕으로 생성된 것을 객체라고 한다.

	// 클래스가 가지고 있는 것 : 데이터 = 멤버필드 (변수와 상수)
	//						기능 = 동작 = 멤버 메서드
	// 						생성자 = 클래스가 객체로 만들어질 때 한번 호출되어서 데이터를 초기화할 수 있는 기능을 가졌다
	// 						** 클래스 이름 = 저장 이름 = 파일 이름 = 생성자이름
	
	// 클래스 구조 : 헤더, 멤버필드, 생성자, 멤버메서드
	// 1. 헤더 : [접근제한자] [클래스종류에 대한 예약어] class 클래스이름{		}
	//	1) 접근제한자 : 클래스, 멤버필드, 멤버메서드, 생성자 모두 맨 처음에 사용할 수 있다.
	//				외부에서 접근할 수 있는 권한을 단계를 말함
	// 		가) public : 누구나 다 접근 가능
	//		나) protected : 같은 패키지와 상속관계에서는 접근 가능			
	//		다) 생략(default) : 같은 패키지에서만 접근가능 
	//		라) private : 외부에서는 절대 접근 불가, 내부멤버(멤버필드, 멤버메서드)끼리만 접근 가능
	...
	//			반환형이 없는 메서드의 형태이다.
	//			[접근제한자] 생성자(클래스 이름)([인자]){ 실행 내용 }
	//			인자가 없는 생성자를 기본 생성자라고 한다.
	//	 생성자 오버로딩 가능
	//	 하나의 클래스에 여러 개의 생성자가 존재할 수 있다. => 생성자 오버로딩
