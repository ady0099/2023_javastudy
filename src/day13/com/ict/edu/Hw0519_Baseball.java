			...
			input: while (true) {
				...
				input2: while (true) {
					...
					try {
						...
						if (intInput2 >= 0 && intInput2 < 10) {
							break input2;
						} else if (intInput2 == intInput1) {
				...
				input3: while (true) {
					...
					try {
						...
						if (!(intInput3 >= 0 && intInput3 < 10)) {
							break input3;
						} else if (intInput3 == intInput1 || intInput3 == intInput2) {
							System.out.println("중복되는 숫자를 입력하셨습니다.");
							continue input3;
					...
					System.out.println("힌트를 확인하시겠습니까? 예(Y), 아니오(N)");
					{
						...
						for (int i = 0; i < rgy.length; i++) {
							if (rgy[i].equalsIgnoreCase(hint)) {
								System.out.println("확인하고 싶으신 자릿수를 입력해주세요. (1~3)");
								hInput = sc.nextInt();
