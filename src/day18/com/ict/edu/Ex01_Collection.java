...
public class Ex01_Collection {
	...
	//		ㅡMap형식 : <K,V>=> key,Value;

	// 최상위 컬렉션은 Collection<E>을 상속받아서 실제로 사용하는 인터페이스들은 : Set{E}, List<E>, Queue<E>
	
	// collection의 주요메서드
	// add(E e) : boolean => 해당 컬렉션에 객체 추가, 성공하면 true, 실패하면 false
	// 						보통은 리턴을 받지 않는다.(set에서는 중복검사 시 받는다.)
	// addAll(Collection <? extends E > c) : boolean
	//		=> 특정 커렉션에 있는 모든 요소들을 다른 컬렉션에 추가
	// clear():vodi; =? 모든 요소 삭제
	// contains(object 0) : boolean =? 인자로 들어온 객체가 포함되어있으면 : true;
	// is Empty() boolean => 해당 컬렉션이 비어있으면 true;
	// Iterator() : Iterator<E> => 컬렉션 안에 존재하는 객체들을 순서대로 접근하기 위해 Iterator 개체를 반환
