...
public class Ex04_Main extends JFrame{
	...
	public Ex04_Main() {
		...
		jrb1.addItemListener(new ItemListener() {
			...
			public void itemStateChanged(ItemEvent e) {
				if(e.getStateChange() == e.SELECTED) {
					canvas.image = Toolkit.getDefaultToolkit().getImage("src/img/java1.png");
					// 캔버스는 한번 그려진 그림은 바꾸지 못한다
					// => 다시 그려 주세용
					canvas.repaint();
				}
		...
		jrb2.addItemListener(new ItemListener() {
			...
			public void itemStateChanged(ItemEvent e) {
				if(e.getStateChange() == e.SELECTED) {
					canvas.image = Toolkit.getDefaultToolkit().getImage("src/img/java2.png");
					canvas.repaint();
				}
