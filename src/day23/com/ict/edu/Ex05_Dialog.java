	...
	public Ex05_Dialog() {
		...
		jb1.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				// 메세지 다이얼 로그 : 사용자에게 정보를 알리는 역할만 한다.(js(자바스크립트 alert())
				...
				JOptionPane.showMessageDialog(getParent(), "방가방가", "MESSAGE 창", JOptionPane.INFORMATION_MESSAGE);
		...
		jb2.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				// OK_CANCEL_OPTAION(확인, 취소)
				 JOptionPane.showConfirmDialog(jp, "정말삭제할까요", "확인창", JOptionPane.YES_NO_OPTION);
			}
		});
		...
		jb3.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				// String msg = JOptionPane.showInputDialog(jp, "나이 : ", 17); << 초기값을 줄 수 있다
			}
		});
		jb4.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				// JOptionPane.showOptionDialog(rootPane, e, getTitle(), ALLBITS, 
				//								ABORT, null, getComponentListeners(), e);
				int res = JOptionPane.showOptionDialog(jp, "방방", "사용자 다이얼로그", 
		...
		jb5.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				// 파일 열기 다이얼 로그
				FileDialog fd = new FileDialog((JFrame)getParent(),"파일열기^o^",FileDialog.LOAD);
				fd.setVisible(true);
		...
		jb6.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				// 파일 저장 다이얼 로그
				FileDialog fd = new FileDialog((JFrame)getParent(),"파일저장ㅍ_ㅍ",FileDialog.SAVE);
				fd.setVisible(true);
				String msg = fd.getDirectory()+fd.getFile();
				System.out.println(msg);
			}
